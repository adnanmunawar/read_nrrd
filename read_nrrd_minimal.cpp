...
public:
    Mat(){}
    Mat(unsigned int a_rows, unsigned int a_cols){
        m_n_rows = a_rows;
        m_n_cols = a_cols;
        data.resize(a_rows * a_cols);
    }

    std::string size(){
        std::string size_str = "[" + std::to_string(m_n_rows) + " x " + std::to_string(m_n_cols) + "]";
        return size_str;
    }
...
std :: vector <Mat > read_slices (std :: ifstream & is, const DataSize & data_size, const int & compression)
{
  ...
  std :: vector <Mat > slices (data_size.m_slices);

  for (int i = 0; i < data_size.m_slices; ++i)
  {
    slices[i] = Mat(data_size.m_width, data_size.m_height);
...
std :: vector < Mat > read_nrrd (const std :: string & filename, const bool & print_header)
{
